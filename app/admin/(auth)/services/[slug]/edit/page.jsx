...
export default function EditServicePage() {
  ...
  // UI helpers
  const handleChange = (field, value) => setService(prev => ({ ...prev, [field]: value }));
  ...
  // SEO Handlers
  const handleSEOChange = (field, value) => setService(prev => ({
    ...prev,
    seo: { ...prev.seo, [field]: value }
  }));

  const handleOGChange = (field, value) => setService(prev => ({
    ...prev,
    seo: { ...prev.seo, openGraph: { ...prev.seo.openGraph, [field]: value } }
  }));
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (service.bannerImage instanceof File) fd.append("bannerImage", service.bannerImage);
      ...
      service.subservices.forEach((sub, idx) => {
        ...
        if (sub.image instanceof File) fd.append(`subservices[${idx}][image]`, sub.image);
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-5xl mx-auto p-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              {service.bannerImage && !(service.bannerImage instanceof File) && (
                <img src={service.bannerImage.secure_url || service.bannerImage} alt="Banner" className="h-20 my-2 rounded border" />
              )}
            </div>
            <Input
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              {service.seo.openGraph.currentImage && (
                <img src={service.seo.openGraph.currentImage} alt="OG Image" className="h-20 my-2 rounded border" />
              )}
            </div>
            <Input
              placeholder="OG URL"
      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-6">
          {service.subservices.length === 0 && (
            <div className="text-muted-foreground text-sm">No subservices yet.</div>
          )}
          {service.subservices.map((sub, idx) => (
            <Card key={idx} className="bg-gray-50 border-2 border-dashed">
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle className="text-base font-semibold">Subservice {idx + 1}</CardTitle>
                  <Button type="button" variant="ghost" size="icon" onClick={() => removeSub(idx)}>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div>
                    <label className="text-sm font-medium">Subservice Image</label>
                    <input type="file" accept="image/*" onChange={e => handleSubFile(idx, e)} />
                    {sub.currentImage && (
                      <img src={sub.currentImage} alt="Subservice" className="h-20 my-2 rounded border" />
                    )}
                  </div>
                  <Input
          ...
          <Button type="button" size="sm" variant="secondary" onClick={addSubservice}>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {service.industries.length === 0 && <div className="text-muted-foreground text-sm">No industries yet.</div>}
          {service.industries.map((ind, idx) => (
            <div key={idx} className="flex gap-2 mb-1">
              ...
              <Input
                placeholder="Detail"
                value={ind.detail}
                onChange={e => handleIndustryChange(idx, "detail", e.target.value)}
                className="flex-1"
              />
              <Button type="button" variant="ghost" size="icon" onClick={() => removeIndustry(idx)}>
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
